...
- `token` (string) - IAM Token. If not set, all `service_account.*` fields below are required.
- `service_account.private_key_file` (string) - Path to the service account private key file (required if `token` is not set).
